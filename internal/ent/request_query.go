...
func (_q *RequestQuery) loadUsageLogs(ctx context.Context, query *UsageLogQuery, nodes []*Request, init func(*Request), assign func(*Request, *UsageLog)) error {
	...
	for _, n := range neighbors {
		fk := n.request_usage_logs
		if fk == nil {
			return fmt.Errorf(`foreign-key "request_usage_logs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "request_usage_logs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
